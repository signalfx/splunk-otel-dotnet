...
#if NET
    [Fact]
    public async Task RuntimeEndpointCanEnableAndReconfigureContinuousProfiler()
    {
        EnableBytecodeInstrumentation();
        SetEnvironmentVariable("SPLUNK_PROFILER_ENABLED", "false");
        SetEnvironmentVariable("SPLUNK_PROFILER_MEMORY_ENABLED", "false");
        SetEnvironmentVariable("SPLUNK_PROFILER_EXPORT_INTERVAL", "500");
        ...
        var appPort = TcpPortProvider.GetOpenPort();
        var appUrl = $"http://localhost:{appPort}";
        var agentPort = TcpPortProvider.GetOpenPort();
        var agentUrl = $"http://localhost:{agentPort}";
        SetEnvironmentVariable("SPLUNK_PROFILER_RUNTIME_CONFIG_ENDPOINT_ENABLED", "true");
        SetEnvironmentVariable("SPLUNK_PROFILER_RUNTIME_CONFIG_ENDPOINT_PORT", agentPort.ToString());

        using var process = StartTestApplication(new() { Arguments = $"--runtime-config-server --urls {appUrl}" });
        ...
        try
        {
            await HealthzHelper.TestAsync($"{appUrl}/alive-check", Output);
            await HealthzHelper.TestAsync($"{agentUrl}/healthz", Output);

            var preEnableCount = await WaitForSettledLogCount(logsCollector);
            var preEnableLogs = logsCollector.GetAllLogs();
            Assert.False(ContainsContinuousCpuRecord(preEnableLogs), "CPU profiler records should not be exported before runtime enable.");
            Assert.False(ContainsAllocationRecord(preEnableLogs), "Allocation profiler records should not be exported before runtime enable.");

            var response = await ConfigureRuntime(
                client,
                agentUrl,
                new Dictionary<string, string?>
                {
                    ...
                    ["SPLUNK_PROFILER_EXPORT_INTERVAL"] = "60000"
                });
            Assert.Contains("\"SPLUNK_PROFILER_EXPORT_INTERVAL\"", response);

            await RunProfilerWork(client, appUrl);
            var logsAfterEnable = await WaitForLogs(
                logsCollector,
                logs =>
                {
                    var postEnableLogs = logs.Skip(preEnableCount).ToArray();
                    return ContainsContinuousCpuRecord(postEnableLogs) && ContainsAllocationRecord(postEnableLogs);
                },
                "enabled CPU and allocation profiler records",
                TimeSpan.FromSeconds(10));

            var postEnableLogs = logsAfterEnable.Skip(preEnableCount).ToArray();
            AssertProfilePeriods(GetContinuousCpuRecords(postEnableLogs), 1000);

            var preIntervalChangeCount = await WaitForSettledLogCount(logsCollector);
            ...
            await ConfigureRuntime(
                ...
                new Dictionary<string, string?>
                {
                    ["SPLUNK_PROFILER_CALL_STACK_INTERVAL"] = "5000"
                });

            await RunProfilerWork(client, appUrl);
            var logsAfterIntervalChange = await WaitForLogs(
                logsCollector,
                logs => ContainsContinuousCpuRecord(logs.Skip(preIntervalChangeCount).ToArray()),
                "CPU profiler records after call stack interval change",
                TimeSpan.FromSeconds(10));

            var postIntervalChangeLogs = logsAfterIntervalChange.Skip(preIntervalChangeCount).ToArray();
            AssertProfilePeriods(GetContinuousCpuRecords(postIntervalChangeLogs), 5000);
        }
        finally
        {
            await StopRuntimeConfigServer(client, appUrl, process!, helper);
        }
    }

    [Fact]
    public async Task RuntimeEndpointCanEnableAndReconfigureSnapshots()
    {
        EnableBytecodeInstrumentation();
        SetEnvironmentVariable("SPLUNK_SNAPSHOT_PROFILER_ENABLED", "false");
        SetEnvironmentVariable("SPLUNK_PROFILER_ENABLED", "false");
        ...
        try
        {
            ...
            var preEnableCount = await WaitForSettledLogCount(logsCollector);
            var preEnableLogs = logsCollector.GetAllLogs();
            Assert.False(ContainsSnapshotRecord(preEnableLogs), "Snapshot profiler records should not be exported before runtime enable.");
            ...
            Assert.Contains("\"SPLUNK_SNAPSHOT_SELECTION_PROBABILITY\"", response);

            await RunProfilerWork(client, appUrl);
            var logsAfterEnable = await WaitForLogs(
                logsCollector,
                logs => ContainsSnapshotRecord(logs.Skip(preEnableCount).ToArray()),
                "enabled snapshot profiler records",
                TimeSpan.FromSeconds(10));

            var postEnableLogs = logsAfterEnable.Skip(preEnableCount).ToArray();
            AssertProfilePeriods(GetSnapshotRecords(postEnableLogs), 300);
            ...
            var logsAfterIntervalChange = await WaitForLogs(
                logsCollector,
                logs => ContainsSnapshotRecord(logs.Skip(preIntervalChangeCount).ToArray()),
                "snapshot profiler records after sampling interval change",
                TimeSpan.FromSeconds(10));

            var postIntervalChangeLogs = logsAfterIntervalChange.Skip(preIntervalChangeCount).ToArray();
            AssertProfilePeriods(GetSnapshotRecords(postIntervalChangeLogs), 100);
        }
        finally
        {
            await StopRuntimeConfigServer(client, appUrl, process!, helper);
