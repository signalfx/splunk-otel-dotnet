...
using System.Text;
using Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.Serializers;
...
namespace Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.Meta
{
    ...
    internal class MetaType : ISerializerProxy
    {
        ...
        internal static bool GetAsReferenceDefault(RuntimeTypeModel model, Type type)
        {
            ...
            {
                if (typeAttribs[i].AttributeType.FullName == "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoContractAttribute")
                {
        ...
        internal void ApplyDefaultBehaviourImpl()
        {
            ...
            for (int i = 0; i < typeAttribs.Length; i++)
            {
                ...
                if (!isEnum && fullAttributeTypeName == "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoIncludeAttribute")
                {
                    int tag = 0;
                ...
                if (fullAttributeTypeName == "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoPartialIgnoreAttribute")
                {
                ...
                if (!isEnum && fullAttributeTypeName == "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoPartialMemberAttribute")
                {
                    if (partialMembers == null) partialMembers = new BasicList();
                    partialMembers.Add(item);
                }

                if (fullAttributeTypeName == "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoContractAttribute")
                {
            ...
            foreach (MemberInfo member in foundList)
            {
                ...
                else if (member is MethodInfo method)
                {
                    ...
                    {
                        CheckForCallback(method, memberAttribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoBeforeSerializationAttribute", ref callbacks, 0);
                        CheckForCallback(method, memberAttribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoAfterSerializationAttribute", ref callbacks, 1);
                        CheckForCallback(method, memberAttribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoBeforeDeserializationAttribute", ref callbacks, 2);
                        CheckForCallback(method, memberAttribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoAfterDeserializationAttribute", ref callbacks, 3);
        ...
        internal static AttributeFamily GetContractFamily(RuntimeTypeModel model, Type type, AttributeMap[] attributes)
        {
            ...
            for (int i = 0; i < attributes.Length; i++)
            {
                ...
                {
                    case "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoContractAttribute":
                        bool tmp = false;
        ...
        private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, AttributeFamily family, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName, ref bool hasConflictingEnumValue, MemberInfo backingMember = null)
        {
            ...
            if (isEnum)
            {
                attrib = GetAttribute(attribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoIgnoreAttribute");
                if (attrib != null)
                {
                    ignore = true;
                }
                ...
                {
                    attrib = GetAttribute(attribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoEnumAttribute");
            ...
            {
                attrib = GetAttribute(attribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoMemberAttribute");
                GetIgnore(ref ignore, attrib, attribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoIgnoreAttribute");
        ...
        private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
        {
            ...
            if (vm != null)
            {
                ...
                {
                    if ((attrib = GetAttribute(attribs, "Splunk.OpenTelemetry.AutoInstrumentation.Vendors.ProtoBuf.ProtoMapAttribute")) != null)
                    {
