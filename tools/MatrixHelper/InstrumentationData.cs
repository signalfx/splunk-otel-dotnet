...
internal static class InstrumentationData
{
    public static Instrumentation[] GetInstrumentations()
    {
        ...
        var instrumentations = new Instrumentation[]
        {
            ...
            new(["MONGODB"], [new InstrumentedComponent("MongoDB.Driver.Core", "2.7.0 and higher"), new InstrumentedComponent("MongoDB.Driver", "3.0.0 and higher")], "Not supported on .NET Framework", "beta", "third-party", [], [new TracesList()], []),
            ...
            new(
                ["NPGSQL"],
                [new InstrumentedComponent("Npgsql", "6.0.0 and higher")],
                "Metrics are not supported on .NET Framework. Trace context propagation is supported only on .NET.",
                "beta",
                "third-party",
                [],
                [
                ...
                [new("OTEL_DOTNET_AUTO_NPGSQL_CONTEXT_PROPAGATION", "Whether the Npgsql instrumentation propagates the W3C traceparent to PostgreSQL through `application_name` for non-multiplexed commands and Npgsql 10 COPY operations.", "false", "boolean", SettingsData.InstrumentationCategory)]),
            new(["ORACLEMDA"], [new InstrumentedComponent("Oracle.ManagedDataAccess.Core", "23.4.0 and higher"), new InstrumentedComponent("Oracle.ManagedDataAccess", "23.4.0 and higher")], "Not supported on ARM64", "beta", "third-party",  [], [new TracesList()], [new("OTEL_DOTNET_AUTO_ORACLEMDA_DATABASE_OPENTELEMETRY_TRACING", "Whether the Oracle Client instrumentation can enable database OpenTelemetry tracing and propagate context to the server.", "true", "boolean", SettingsData.InstrumentationCategory), new("OTEL_DOTNET_AUTO_ORACLEMDA_SET_DBSTATEMENT_FOR_TEXT", "Whether the Oracle Client instrumentation can pass SQL statements through the `db.statement` attribute. Queries might contain sensitive information. If set to `false`, `db.statement` is recorded only for executing stored procedures.", "false", "boolean", SettingsData.InstrumentationCategory)]),
            ...
            new(["NLOG"], [new InstrumentedComponent("NLOG", "5.0.0 to 7.0.0")], null, "beta", "community", [], [new LogLists()], [new Setting("OTEL_DOTNET_AUTO_LOGS_ENABLE_NLOG_BRIDGE", "Enables `NLog` bridge. When `NLog` logs bridge is enabled, application logs are exported in OTLP format in addition to being written into their currently configured destination.", "false", "boolean", SettingsData.InstrumentationCategory)]),
        };
