...
public static class SettingsData
{
    ...
    public static Setting[] GetSettings()
    {
        var settings = new Setting[]
        {
            ...
            new("OTEL_DOTNET_AUTO_EXCLUDE_PROCESSES", "Names of the executable files that you don't want the profiler to instrument. Supports multiple semicolon-separated values, for example: `ReservedProcess.exe;powershell.exe`. Notice that applications launched using dotnet MyApp.dll have process name `dotnet` or `dotnet.exe`. Can't be set using the web.config or app.config files.", string.Empty, GeneralCategory, "string"),
