...
internal static class SettingsData
{
    ...
    public static Setting[] GetSettings()
    {
        var settings = new Setting[]
        {
            ...
            new("OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE", "The aggregation temporality to use on the basis of instrument kind. The supported options are `Cumulative` for all instrument kinds and `Delta` for Counter, Asynchronous Counter, and Histogram instrument kinds. `LowMemory`, from the OpenTelemetry specification, is not supported. Use `Cumulative` for UpDownCounter and Asynchronous UpDownCounter instrument kinds.", "Cumulative", "string", ExporterCategory),
