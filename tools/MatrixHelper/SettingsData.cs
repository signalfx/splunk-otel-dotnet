...
internal static class SettingsData
{
    ...
    public static Setting[] GetSettings()
    {
        var settings = new Setting[]
        {
            ...
            new("SPLUNK_REALM", "The Splunk Observability Cloud realm where the telemetry should be sent. For example, us0 or us1. Defaults to none, which means that data goes to a Splunk OpenTelemetry Collector deployed on localhost. When set to none, all telemetry data (traces, metrics, logs, profiles) is sent to the Splunk OpenTelemetry Collector. When set to any value other than none, only traces and metrics are sent directly to the specified Splunk Observability Cloud realm. Logs and profiling data are not sent.", string.Empty, "string", ExporterCategory),
