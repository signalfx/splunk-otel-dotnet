...
internal static class SettingsData
{
    ...
    public static Setting[] GetSettings()
    {
        var settings = new Setting[]
        {
            ...
            new("OTEL_DOTNET_AUTO_NETFX_REDIRECT_ENABLED", "Activates immediate redirection of the assemblies used by the automatic instrumentation on the .NET Framework. The default values is `true`. Can't be set using the web.config or app.config files. `OTEL_DOTNET_AUTO_NETFX_REDIRECT_ENABLED` is deprecated use `OTEL_DOTNET_AUTO_REDIRECT_ENABLED` instead", "true", "boolean", GeneralCategory),
            new("OTEL_DOTNET_AUTO_REDIRECT_ENABLED", "Activates immediate redirection of the assemblies used by the automatic instrumentation on the .NET Framework. The default values is `true`. Can't be set using the web.config or app.config files.", "true", "boolean", GeneralCategory),
            ...
            new("OTEL_EXPORTER_OTLP_CERTIFICATE", "Path to the CA certificate file (PEM format) used to verify the server's TLS certificate. Supported only on .NET.", string.Empty, "string", ExporterCategory),
            new("OTEL_EXPORTER_OTLP_CLIENT_CERTIFICATE", "Path to the client certificate file (PEM format) for mTLS authentication. Supported only on .NET.", string.Empty, "string", ExporterCategory),
            new("OTEL_EXPORTER_OTLP_CLIENT_KEY", "Path to the client private key file (PEM format) for mTLS authentication. Supported only on .NET.", string.Empty, "string", ExporterCategory),
            ...
            new("SPLUNK_SNAPSHOT_SAMPLING_INTERVAL", "Sampling interval for snapshot collections, in milliseconds.", "40", "int", ProfilingCategory),
            ...
            new("SPLUNK_OPAMP_REMOTE_CONFIG", "Set to `true` to enable remote configuration features.", "false", "boolean", OpAmpCategory),
        };
