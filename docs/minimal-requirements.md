# Minimal Recommendation to Execute splunk-otel-dotnet

The minimal recommended resources to execute `splunk-otel-dotnet` are:

- `2 CPU`
- `512 MB RAM`

These values are only a minimal recommendation and should be treated as a
starting point, not as a universal sizing guideline for every deployment.

Actual resource requirements can vary depending on application load,
